...
//for spoofing the request DNS packet, I need to set the Answer RRS to be the number of answers and generate the random transaction ID, also I should structure the answers and put it at the end of the DNS packet
...
//struct of the DNS answer section
struct dnsanswer {
    unsigned short int a_name;            //2bytes,
    unsigned short int a_type;            //For most cases, the type is 0X0001, and the final part refers to IP address
    unsigned short int a_class;
    unsigned int a_ttl;                   //4bytes, ttl
    unsigned short int a_length;               //2bytes, data length
    unsigned int a_address;              //4bytes, IP address
};
...
uint16_t check_udp_sum(uint8_t *buffer, int len)
{
	...
	return (uint16_t)(~sum);                 //get the negation of sum
...
//query string
    strcpy(data,"\5hello\7example\3com");
