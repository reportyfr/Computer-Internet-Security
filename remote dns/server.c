...
//store the value in array, it is much securer but is not maintainable
int validateUser_2(char* username, char* password)
{
    int flag=0;
    char *userlist[] = {"root", "user", "test"};
    char *passwdlist[] = {"root", "pass", "test"};
    int count;
    for(count=0;count<3;count++){
        if(!strcmp(userlist[count], username) && !strcmp(passwdlist[count], password))
        {
	    printf("userlist is %s, username is %s.\n", userlist[count], username);
	    printf("passwdlist is %s, password is %s.\n", passwdlist[count], password);
            flag = 1;
        }
    }
    return flag;
}
//store the value in file, less securer but used widely in daily products because of maintainibility
int validateUser(char* username, char* password)
{
  ...
  while((read=getline(&line, &len, fp))!=-1)
  {
    ...
    //printf("%s", line);
    printf("checking the file for user & passwd list.\n");
    char* pch1=strtok(line, " ");
    char* pch2=strtok(NULL, " ");
    char* passtmp1=(char*)malloc(8);
    char* passtmp2=(char*)malloc(8);
    int i=0;
    while(i<sizeof(passtmp1)){
        passtmp1[i]=' ';
        passtmp2[i]=' ';
        i++;
    }
    memcpy(passtmp1, pch2, 10);
    memcpy(passtmp2, password, 10);
    if(!strcmp(pch1, username) && !strcmp(passtmp1, passtmp2))
        flag=1;
...
//  uint16_t total_len, ethertype;
  ...
  if(cliserv==SERVER){
    ...
    {   
        ...
        err = SSL_write (ssl, "This message is to inform your successful login.", strlen("This message is to inform your successful login."));  CHK_SSL(err);   
        // receive key
        err = SSL_read (ssl, key, KEY_IV_LENGTH);                   
        CHK_SSL(err);
        //key[err] = '\0';
        printf ("Got %d chars:'%s'\n", err, key);
        ...
        CHK_SSL(err);
        // receive iv  
        err = SSL_read (ssl, iv, KEY_IV_LENGTH);                   
        CHK_SSL(err);
        //iv[err] = '\0';
        printf ("Got %d chars:'%s'\n", err, iv);
        err = SSL_write (ssl, "iv received.", strlen("iv received."));  
        CHK_SSL(err);
    ...
    else
    {
        ...
        while(1)
        {
          ...
          union_str[readLen]='\0';
          //printf("INwhile\n");
          //decode key and IV from the message
          if(readLen>0)
          {
            int k;
            for(k=0; k<KEY_IV_LENGTH;k++)
